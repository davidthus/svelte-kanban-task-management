<script>
	...
	let hover;
	let activeBoard;
	const unsubscribe = data.activeBoard.hasOwnProperty('subscribe')
		? data.activeBoard?.subscribe((value) => {
				activeBoard = value;
		  })
		: null;
	const baseStyles =
	...
	if (hover) state = buttonStateTypes.HOVER;
	if (activeBoard === board.name || data.activeBoard === board.name)
		state = buttonStateTypes.ACTIVE;
...
<li
	class={`${baseStyles} ${data.activeBoard === board.name && activeStyles}`}
	on:mouseenter={() => {
		hover = true;
	}}
	on:mouseleave={() => {
